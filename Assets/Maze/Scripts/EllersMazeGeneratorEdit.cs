...
{
    public static IEnumerable<(int, T)> Enumerate2<T>(
...
public class EllersMazeGeneratorEdit: MonoBehaviour
{
    ...
    [SerializeField] int floor_yvalue;
    ...
    private void SpawnEllersMaze()
    {
      ...
      //  WorldText_Row(first_row, Color.white, 0);
        ...
        for (int z = 0; z < gridSpawner.height; z++)
        {
            ...
            {
                PlaceFloorExtra(IncreaseAxisRow(1, row), Vector3.right);
            }
            //Step (5) Finalize Row:
            if (z == gridSpawner.height - 1)
            {
                PlaceFloorExtra(IncreaseAxisRow(1, row), Vector3.back);
    ...
    private void BottomWalls(List<(int[], int)> row, out List<int[]> bottomWallLocations)
    {
        ...
        foreach (var set_list in RowSortedBySet(row_copy))
        {
            ...
            foreach (var (i, cell) in shuffledSets.Enumerate2())
            {
                if (i == 0)
                {
...
; PlaceWall_XZ(Vector3.right, coord[0], coord[1]);
    ...
    private List<(int[], int)> EmptyCells_NewRow(List<(int[], int)> row, List<int[]> empty_cells)
    {
        ...
        foreach (var (i, cell) in row.Enumerate2())
        {
    ...
    private void PlaceWall_XZ(Vector3 dir, int x, int z)
    {
        ...
        int randomIndex = UnityEngine.Random.Range(0, wall.Length);
        GameObject.Instantiate(wall[randomIndex], position, Quaternion.LookRotation(dir));
    }

    private void PlaceFloorExtra(List<(int[], int)> row, Vector3 dir)
    {
        foreach (var cellTup in row)
        {
            var coords = cellTup.Item1;

            PlaceFloor(dir, coords[0], coords[1]);
        }
    }
    private void PlaceFloor(Vector3 dir, int x, int z)
    {
        Vector3 position = gridXZ.GetWorldPosition(x, z);
        position.y = floor_yvalue;

        int randomIndex = UnityEngine.Random.Range(0, floor.Length);
        GameObject.Instantiate(floor[randomIndex], position, Quaternion.LookRotation(dir));
    }
